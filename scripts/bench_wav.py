...
def main() -> int:
    ...
    cfg = config.load()
    transcriber = asr.Transcriber(model_dir=model, quantized=False, num_threads=os.cpu_count() or 4,
                                  homophones=args.hr)
    ...
    for u in utterances:
        if u.text:
            print(f"[{int(u.start) // 60}:{int(u.start) % 60:02d}] {u.speaker} ({u.lang}) {u.text}")

    print(f"\nmodel={model.name} hr={'on' if args.hr else 'off'} utterances={len(utterances)} "
          f"speech={audio_seconds:.1f}s wall={elapsed:.1f}s rtf={elapsed / audio_seconds:.2f}")
